...
router.post('/lfd-home-ordering/{{version}}/action10/do-you-have-symptoms', function (req, res) {
    let symptoms = req.session.data['do-you-have-symptoms']
    if (symptoms == "No" ) {
      res.redirect('/lfd-home-ordering/{{version}}/coronavirus-account')
    } else {
      res.redirect('/lfd-home-ordering/{{version}}/different-test')
    }

})

router.post('/lfd-home-ordering/{{version}}/action11/contact-with-positive', function (req, res) {
  let contactWithPositive = req.session.data['contact-with-positive']
  if (contactWithPositive == "No" ) {
    res.redirect('/lfd-home-ordering/{{version}}/work-from-home')
  } else {
    res.redirect('/lfd-home-ordering/{{version}}/daily-contact-testing')
  }

})

// router.post('/lfd-home-ordering/{{version}}/action10/country', function (req, res) {
//     let country = req.session.data['country']
//     if (country == "England" ) {
//       res.redirect('/lfd-home-ordering/{{version}}/coronavirus-account')
//     } else if (country == "Scotland" ) {
//       res.redirect('/lfd-home-ordering/{{version}}/scottish-isles')
//     } else {
//       res.redirect('/lfd-home-ordering/{{version}}/country-test-unavailable')
//     }

// })

router.post('/lfd-home-ordering/{{version}}/action10/work-from-home', function (req, res) {
  let workFromHome = req.session.data['work-from-home']
  if (workFromHome == "Yes" ) {
    res.redirect('/lfd-home-ordering/{{version}}/workplace-testing')
  } else {
    res.redirect('/lfd-home-ordering/{{version}}/lft-unavailable')
  }

})

router.post('/lfd-home-ordering/{{version}}/action10/scottish-isles', function (req, res) {
    let scottishIsles = req.session.data['scottish-isles']
    if (!scottishIsles ) {
      res.redirect('/lfd-home-ordering/{{version}}/scottish-isles-error')
...
router.post('/lfd-home-ordering/{{version}}/action10/workplace-testing', function (req, res) {
    let workplaceTesting = req.session.data['workplace-testing']
    if (workplaceTesting == "No" ) {
      res.redirect('/lfd-home-ordering/{{version}}/coronavirus-account')
    } else {
      res.redirect('/lfd-home-ordering/{{version}}/collect-your-tests')
    }

})

router.post('/lfd-home-ordering/{{version}}/action10/coronavirus-account', function (req, res) {
  let signin = req.session.data['coronavirus-account']
  if (signin == "Yes") {
    res.redirect('/lfd-home-ordering/{{version}}/user-account/login-email')
  } else {
    res.redirect('/lfd-home-ordering/{{version}}/name')
  }

})

router.post('/lfd-home-ordering/{{version}}/user-account/action10/login-email', function (req, res) {
  let loginEmail = req.session.data['email-address']

  if (loginEmail == "user@testing.co.uk"){
    res.redirect('/lfd-home-ordering/{{version}}/user-account/enter-password')
  } else {
    res.redirect('/lfd-home-ordering/{{version}}/user-account/create-password')
  }

})

router.post('/lfd-home-ordering/{{version}}/action10/create-password', function (req, res) {
  let password = req.session.data['password']
  let confirmPassword = req.session.data['confirm-password']
  if (password == "" || confirmPassword == "") {
    res.redirect('/lfd-home-ordering/{{version}}/user-account/create-password-error')
  } else {
    res.redirect('/lfd-home-ordering/{{version}}/user-account/check-email')
  }
})

router.post('/lfd-home-ordering/{{version}}/action10/check-mobile', function (req, res) {
  let securityCode = req.session.data['security-code']
  if (securityCode == "") {
    res.redirect('/lfd-home-ordering/{{version}}/user-account/check-mobile-error')
  } else {
    res.redirect('/lfd-home-ordering/{{version}}/user-account/agreement')
  }
})

router.post('/lfd-home-ordering/{{version}}/user-account/action10/home-page', function (req, res) {
  let loginEmail = req.session.data['email-address']

  if (loginEmail == "user@testing.co.uk"){
    res.redirect('/lfd-home-ordering/{{version}}/check-your-answers')
  } else {
    res.redirect('/lfd-home-ordering/{{version}}/name')
  }
...
router.post('/lfd-home-ordering/{{version}}/action10/email-address', function (req, res) {
    let emailAddress = req.session.data['email']
    if (emailAddress == "No" ) {
        res.redirect('/lfd-home-ordering/{{version}}/call-us')
    } else {
        res.redirect('/lfd-home-ordering/{{version}}/mobile-number')
    }

})

router.post('/lfd-home-ordering/{{version}}/action10/contact-with-positive', function (req, res) {
    let contactWithPositive = req.session.data['contact-with-positive']
    if (contactWithPositive == "No" ) {
        res.redirect('/lfd-home-ordering/{{version}}/dct-unavailable')
    } else {
        res.redirect('/lfd-home-ordering/{{version}}/notified-how')
    }

})

router.post('/lfd-home-ordering/{{version}}/action10/dct-opt-in', function (req, res) {
    let dctOptIn = req.session.data['dct-opt-in']
    if (dctOptIn == "No" ) {
        res.redirect('/lfd-home-ordering/{{version}}/self-isolate')
    } else {
        res.redirect('/lfd-home-ordering/{{version}}/name')
    }
...
router.post('/lfd-home-ordering/{{version}}/action10/country', function (req, res) {
    let country = req.session.data['country']
    if (country == "England" ) {
        res.redirect('/lfd-home-ordering/{{version}}/nhs-testing-programme')
    } else {
        res.redirect('/lfd-home-ordering/{{version}}/check-your-answers')
    }
...
router.post('/lfd-home-ordering/{{version}}/action10/nhs-testing-programme', function (req, res) {
    let programme = req.session.data['nhs-testing-programme']
    if (programme == "Yes" ) {
        res.redirect('/lfd-home-ordering/{{version}}/work-area')
    } else {
        res.redirect('/lfd-home-ordering/{{version}}/check-your-answers')
    }
...
router.post('/lfd-home-ordering/{{version}}/action10/work-area', function (req, res) {
    ...
    if (workArea == "Community pharmacy" || workArea == "Dentistry" || workArea == "General practice" || workArea == "Optometry" || workArea == "Other" ) {
        res.redirect('/lfd-home-ordering/{{version}}/work-postcode')
    } else {
        res.redirect('/lfd-home-ordering/{{version}}/trust')
    }

})

router.post('/lfd-home-ordering/{{version}}/action10/home-address-question', function (req, res) {
    let deliveryAddressSame = req.session.data['delivery-address-same']
    if (deliveryAddressSame == "No" ) {
        res.redirect('/lfd-home-ordering/{{version}}/delivery-postcode')
    } else {
        res.redirect('/lfd-home-ordering/{{version}}/confirm-email-address')
    }
...
router.post('/lfd-home-ordering/{{version}}/action10/do-you-have-symptoms', function (req, res) {
  let symptoms = req.session.data['do-you-have-symptoms']
  if (symptoms == "No" ) {
    res.redirect('/lfd-home-ordering/{{version}}/country')
  } else {
    res.redirect('/lfd-home-ordering/{{version}}/different-test')
  }
...
router.post('/lfd-home-ordering/{{version}}/action10/country', function (req, res) {
  let country = req.session.data['country']
  if (country == "England" ) {
    res.redirect('/lfd-home-ordering/{{version}}/contact-with-positive')
  } else {
    res.redirect('/lfd-home-ordering/{{version}}/country-test-unavailable')
  }
...
router.post('/lfd-home-ordering/v3/action10/test-choice', function (req, res) {
  ...
  if (testChoice == "Get a one off test at a community test site" ) {
    res.redirect('/pre-registration/{{version}}/')
  } else if (testChoice == "Collect tests to take home" ) {
    res.redirect('/lfd-collection-registration/{{version}}/nhs-account')
  } else {
